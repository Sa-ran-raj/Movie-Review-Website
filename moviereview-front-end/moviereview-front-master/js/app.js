                ...
                <div class="card h-100" onclick="openMovieModal('${movie.id}')">
                    ...
                    <div class="card-body">
                        ...
                        <div class="review-rating">
                            <span class="rating-value ${ratingColorClass}">${averageRating}</span>
...
function openMovieModal(movieId) {
    fetch(`${apiBaseUrl}/${movieId}`)
        .then(response => response.json())
        .then(movie => {
            // Calculate the average rating here
            const averageRating = calculateAverageRating(movie.reviews);
            // Add this line to define the ratingColorClass
            const ratingColorClass = getRatingColorClass(averageRating);
            ...
            document.getElementById('movie-rating').innerHTML = `<span class="${ratingColorClass}">${averageRating}</span> ${getStarRating(averageRating)}`;
...
function calculateAverageRating(reviews) {
    ...
    return (sum / reviews.length).toFixed(1); // Returns average with one decimal place
}
